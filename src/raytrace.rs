...
use model::{SceneNode,Collidable};
use std::ops::{Add,Mul};
...
impl Color {
    ...
    pub fn as_rgb(&self) -> Rgb<u8> {
        Rgb([(self.r * 255.0).round() as u8, (self.g * 255.0).round() as u8, (self.b * 255.0).round() as u8])
    }
...
impl Raytracer {
    ...
    fn trace_ray(&self, width: u32, height: u32, ray: &Ray, x: u32, y: u32, fx: f32, fy: f32) -> Color {
...
//}
 ...
 fn get_background_color(x: u32, y: u32, width: u32, height: u32) -> Color {
     ...
     return Color::new(r_rate * height_rate, g_rate * height_rate, b_rate * height_rate);
 }
